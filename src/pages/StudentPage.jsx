...
import { CalendarPlus, ChevronDown, ExternalLink, FileUp, KeyRound, Plus, RefreshCw, ShieldCheck } from 'lucide-react'
...
export default function StudentPage(){
  ...
  const submitPlan=async(e)=>{
    ...
    setBusy(true)
    const payload={...form,period:Number(form.period),student_id:profile.id,device_purpose:form.use_device?form.device_purpose.trim():null}
  ...
  return <div className="page">
    <section className="dashboard-heading">
      <div><span className="eyebrow">KẾ HOẠCH CỦA EM</span><h1>Chào {profile?.full_name}</h1><p>MSHS: <strong>{profile?.mshs}</strong> · Lập kế hoạch trước, thực hiện có mục tiêu, rồi nhìn lại kết quả.</p></div>
      <div className="button-row"><button className="button ghost" onClick={()=>setShowPassword(true)}><KeyRound size={17}/> Đổi mật khẩu</button><button className="button primary" onClick={()=>setShowForm(!showForm)}><CalendarPlus size={18}/> Đăng ký giờ tự học</button></div>
    </section>

    <section className="student-summary">
      <Stat label="Kế hoạch sắp tới" value={upcoming.length}/><Stat label="Đúng hạn" value={`${onTimeRate}%`}/><Stat label="Đã hoàn thành" value={completedCount}/><Stat label="Minh chứng" value={evidenceCount}/>
    </section>
    ...
    <section className="section-block">
      <div className="section-title"><div><h2>Sắp tới</h2><p>Các kế hoạch từ hôm nay trở đi. Kế hoạch tương lai có thể chỉnh sửa.</p></div><button className="icon-button" onClick={load} title="Làm mới"><RefreshCw size={18}/></button></div>
      {upcoming.length===0?<EmptyState text="Chưa có kế hoạch sắp tới."/>:<div className="plan-grid">{upcoming.map(p=><PlanCard key={p.id} plan={p} reflection={reflections[p.id]} evidence={evidence[p.id]||[]} onOpen={()=>setOpenPlan(p)}/>)}</div>}
    </section>

    <section className="section-block">
      <div className="section-title"><div><h2>Lịch sử gần đây</h2><p>Sau giờ tự học, cập nhật kết quả và minh chứng nếu có.</p></div></div>
      {past.length===0?<EmptyState text="Chưa có lịch sử tự học."/>:<div className="plan-grid">{past.slice(0,15).map(p=><PlanCard key={p.id} plan={p} reflection={reflections[p.id]} evidence={evidence[p.id]||[]} onOpen={()=>setOpenPlan(p)}/>)}</div>}
    </section>

    {openPlan&&(openPlan.study_date>todayISO()?<EditPlanModal plan={openPlan} onClose={()=>setOpenPlan(null)} onSaved={()=>{setOpenPlan(null);load()}}/>:<ReflectionModal plan={openPlan} existing={reflections[openPlan.id]} evidence={evidence[openPlan.id]||[]} onClose={()=>setOpenPlan(null)} onSaved={()=>{setOpenPlan(null);load()}}/>)}
...
function PlanForm({form,setForm,onSubmit,busy}){
  ...
  return <form className="card plan-form" onSubmit={onSubmit}>
    ...
    <div className="form-grid three"><div><label>Ngày tự học *</label><input type="date" min={todayISO()} value={form.study_date} onChange={e=>update('study_date',e.target.value)} required/>{form.study_date&&<small className={days<=0?'text-warning':'text-success'}>{days<=0?'Đăng ký trong ngày sẽ được đánh dấu trễ.':days===1?'Còn 1 ngày — vẫn đúng hạn.':`Còn ${days} ngày — đăng ký đúng hạn.`}</small>}</div><div><label>Tiết *</label><select value={form.period} onChange={e=>update('period',e.target.value)}>{Array.from({length:9},(_,i)=>i+1).map(n=><option key={n} value={n}>Tiết {n}</option>)}</select></div><div><label>Mức ưu tiên *</label><select value={form.priority} onChange={e=>update('priority',e.target.value)}>{priorityOptions.map(x=><option key={x}>{x}</option>)}</select></div></div>
    ...
    <div className="toggle-row"><label className="switch"><input type="checkbox" checked={form.use_device} onChange={e=>update('use_device',e.target.checked)}/><span/></label><div><strong>Sử dụng thiết bị điện tử</strong><small>Nếu có, cần đăng ký trước ít nhất 1 ngày và ghi đúng mục đích.</small></div></div>
...
function PlanCard({plan,reflection,evidence,onOpen}){
  const status=registrationStatus(plan.study_date,plan.created_at)
  return <article className="card plan-card" onClick={onOpen}>
    <div className="plan-card-top"><span className="date-chip">{formatDate(plan.study_date)} · Tiết {plan.period}</span><StatusBadge value={status}/></div>
    <h3>{plan.subject}</h3><p>{plan.task}</p>
    <div className="plan-meta"><span>Ưu tiên: <StatusBadge value={plan.priority}/></span><span>{plan.use_device?'💻 Có thiết bị':'Không thiết bị'}</span></div>
    <div className="plan-footer"><span>{reflection?<StatusBadge value={reflection.completion_status}/>:<span className="muted-text">Chưa cập nhật kết quả</span>}</span><span>{evidence.length?`📎 ${evidence.length} minh chứng`:(plan.study_date>todayISO()?'Chỉnh sửa kế hoạch':'Cập nhật kết quả')} <ChevronDown size={15}/></span></div>
  </article>
}
...
function EditPlanModal({plan,onClose,onSaved}){
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="modal" onMouseDown={e=>e.stopPropagation()}>
    ...
    <div className="form-grid two"><div><label>Loại hoạt động</label><select value={form.activity_type} onChange={e=>update('activity_type',e.target.value)}>{activityOptions.map(x=><option key={x}>{x}</option>)}</select></div><div><label>Môn / nội dung</label><select value={form.subject} onChange={e=>update('subject',e.target.value)}>{subjectOptions.map(x=><option key={x}>{x}</option>)}</select></div></div>
    <label>Nhiệm vụ cụ thể</label><textarea rows="3" maxLength={1000} value={form.task} onChange={e=>update('task',e.target.value)}/><label>Mục tiêu cuối tiết</label><textarea rows="2" maxLength={1000} value={form.goal} onChange={e=>update('goal',e.target.value)}/><label>Mức ưu tiên</label><select value={form.priority} onChange={e=>update('priority',e.target.value)}>{priorityOptions.map(x=><option key={x}>{x}</option>)}</select>
    <div className="toggle-row"><label className="switch"><input type="checkbox" checked={form.use_device} onChange={e=>update('use_device',e.target.checked)}/><span/></label><div><strong>Sử dụng thiết bị điện tử</strong><small>Nếu bật, ghi rõ mục đích.</small></div></div>{form.use_device&&<input maxLength={500} value={form.device_purpose} onChange={e=>update('device_purpose',e.target.value)} placeholder="Mục đích sử dụng"/>}
    <label>Nếu hoàn thành sớm</label><select value={form.fallback_activity} onChange={e=>update('fallback_activity',e.target.value)}>{fallbackOptions.map(x=><option key={x}>{x}</option>)}</select>
    {msg&&<div className="form-error">{msg}</div>}<div className="form-actions"><button className="button ghost" onClick={onClose}>Đóng</button><button className="button primary" onClick={save} disabled={busy}>{busy?'Đang lưu…':'Lưu điều chỉnh'}</button></div>
  </div></div>
}
...
function ReflectionModal({plan,existing,evidence,onClose,onSaved}){
  ...
  const save=async()=>{
    ...
    if(link.trim()){
      try{new URL(link.trim())}catch{setBusy(false);return setMsg('Liên kết minh chứng chưa hợp lệ.')}
    }
    if(file){
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="modal" onMouseDown={e=>e.stopPropagation()}>
    ...
    <div className="evidence-block"><h3>Minh chứng <span className="muted-text">(tùy chọn, tối đa 3)</span></h3>{evidence.map(x=><span key={x.id}><button type="button" className="evidence-item" onClick={()=>openEvidence(x)}>{x.kind==='link'?'🔗':'📎'} {x.display_name||'Minh chứng'} <ExternalLink size={14}/></button><button type="button" className="evidence-item" onClick={()=>removeEvidence(x)}>✕</button></span>)}
...
function ChangePasswordModal({mshs,onClose}){
  ...
  const save=async()=>{
    ...
    setBusy(true)
    const {error:verifyError}=await supabase.auth.signInWithPassword({email:studentInternalEmail(mshs),password:current})
    if(verifyError){setBusy(false);return setMsg('Mật khẩu hiện tại chưa đúng.')}
    const {error}=await supabase.auth.updateUser({password:pw})
    setBusy(false)
    if(error)return setMsg('Không thể đổi mật khẩu. Hãy thử lại.')
    setCurrent('');setPw('');setConfirm('');setMsg('✓ Đã đổi mật khẩu thành công.')
  }
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="modal small" onMouseDown={e=>e.stopPropagation()}><div className="modal-head"><div><span className="eyebrow">BẢO MẬT TÀI KHOẢN</span><h2>Đổi mật khẩu</h2></div><button className="icon-button" onClick={onClose}>✕</button></div><label>Mật khẩu hiện tại</label><input type="password" value={current} onChange={e=>setCurrent(e.target.value)} autoComplete="current-password"/><label>Mật khẩu mới</label><input type="password" value={pw} onChange={e=>setPw(e.target.value)} autoComplete="new-password"/><div className="password-rules visible">{checks.map(i=><div key={i.key} className={i.ok?'rule-ok':'rule-pending'}>{i.ok?'✓':'○'} <span>{i.label}</span></div>)}</div><label>Nhập lại mật khẩu mới</label><input type="password" value={confirm} onChange={e=>setConfirm(e.target.value)} autoComplete="new-password"/>{msg&&<div className={msg.startsWith('✓')?'notice':'form-error'}>{msg}</div>}<div className="form-actions"><button className="button ghost" onClick={onClose}>Đóng</button><button className="button primary" onClick={save} disabled={busy}>{busy?'Đang đổi…':'Đổi mật khẩu'}</button></div></div></div>
}
